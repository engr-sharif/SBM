...
| **The road drain's pipe in the Clear Lake trench (E943D) is the 30-in HDPE that conveys Green Pond** (J. Lucero / Carter, 3–4 Sep 2026); the dual 24-in are the ones in disrepair | The CAD draws all three lines 2.35 ft apart, which said "24-in" to the builder; the field team's word wins. `size_in: 30` since 2026-09-08; nothing computes with it until inverts arrive |
| **"Clear water overlays" clears EVERYTHING the water tools drew** — the analysis, every route it traced, every raindrop — as one undo entry (2026-09-08) | The engineer: "I hit clear water overlays and nothing really happens". The v15 rule that traced routes are the user's features and survive the analysis stays for the card's slider and hide/show; the clear button and the card's ✕ are the way OUT |
...
| **The logger's own contact remark LEADS, and nothing reconciles the two statements** (2026-09-09) | "How deep is the waste here" has two answers in the logs — the remark written on the rig and the strata rows read back — and 18 of the 44 holes disagree. The remark leads because every hole has one and a person looking at the core wrote it. The log draws both lines, the flags name each difference, and `LOGS` is the sheet they get reconciled on. Averaging them or preferring the "cleaner" source would hide exactly the thing he has to look at |
| **The older field-interpreted waste depth spreadsheet is RETIRED** (2026-09-09) | "The logs are approved and usable for our need; the spreadsheet might be answering something else entirely." It disagreed with the logs on 32 of 44 holes because its basis was blow counts, pH and soil type rather than the logged contact. Its two dataset columns are deleted by `tools/build_borings.py`, the seed builder no longer reads the workbook, and nothing in the app draws it. Site lithology / geologic maps (USGS Clear Lake volcanics) as a layer are a later, low-priority item (open item 0c) |
...
| **Both 24-in barrels are the impoundment's discharge, and the discharge route is the conduit CHAIN, not a raindrop** (ruling, 5 Sep 2026) | His words: *"for the Herman impoundment you are using the culvert discharge pipes but there are 2 pipes and you only used one … make sure the system knows that the water flows through the pipes and out to Clear Lake; right now I think it shows that it goes directly and makes its own path"*. Two faults: the payload connected only the NORTH barrel to EA's drawn storm line, so the SOUTH one — the lower invert, the one the water leaves through — ended 13 ft short of anything; and the card's route was a raindrop dropped at the north pipe's plotted west end, which is a terrain analysis that happens to meet a pipe. `js/water.js` `pipeChainRoute()` walks the network from the sandbag wall to the Clear Lake outfall — **zero ground** before the outfall, then 137 ft overland into the lake. (The links themselves were superseded the next day; see the row below.) Every Herman stage, the drainage identity and every catchment acreage were unchanged; the one number that moved was the raindrop's pipe length, 813.3 → 812.8 ft |
| **The two Herman barrels do NOT merge — three pipes lie in one trench, and the overflow follows them** (ruling, 6 Sep 2026) | His words: *"the two overflow outlets that you have merging into one, that's not the case, we have two pipes that run in parallel with each other that take the overflow from the two overflow pipes out to Clear Lake … there are three pipes that flow along that channel, the two for the Herman impoundment and one that is used by the Frog and Green pond overflow — that's the far south one, which you have solved."* The CAD agreed and the builder had misread it: `V-STRM-STRC` draws THREE 783-ft lines from the sandbag wall to the shore, 2.35 ft apart (the outside diameter of 24-in corrugated HDPE) where a double line with a centreline would be 2.0 ft wide in total. Each barrel now runs to Clear Lake in its own drawn pipe — `herman_main_n` on E943E, `herman_main_s` on E943C — the road drain's `storm_main_upper/lower` move to E943D, and `pipe_to_main` / `pipe_to_main_s` / `storm_main_east` are gone. Still 44 nodes / 27 conduits (17 CAD/survey, 10 inferred). ONE shared `outfall` node, deliberately, so the drainage map's *Clear Lake outfall* stays one 282-acre catchment. The one number that moved is the raindrop's pipe length, 812.8 → **812.2 ft**; the three outlet areas, `herman_pipe_s`'s 37.90-ac through-area and the 100/100 raindrop identity are unchanged, which is the proof the rebuild is right |
| **Every conduit leg follows the pipe EA drew, and the rim overflow appears when the slider reaches the rim** (ruling, 6 Sep 2026) | He was looking at the pipe discharge route at 1,341.55 "discharging". Two things: a leg was drawn as a straight line between its two nodes, so the route left the drawn pipe wherever the CAD bends and read as a flow making its own path — in 2D, as the 3D tube and as the animated particles it now follows the conduit's own polyline; and "not traced" was the wrong answer above the rim, because there the water really is going over. From the rim spill up the rim overflow is traced automatically (once, cached) and shown BESIDE the pipe route as a normal solid animated flow, "over the rim at 1,343.84 ft", disappearing again below the rim. The dashed what-if button stays for the different question of the drains being blocked |
...
| The matrix also runs on **GitHub Actions** (`.github/workflows/matrix.yml`), on pull requests | Seven jobs in parallel put the whole matrix at the length of its slowest step. The repository is private, so it spends Actions minutes — the workflow says so |

| **The page is not a scroll container, and the folder build is field-aware** (v19.1) | His words, from an iPhone on GitHub Pages: *"I'm not sure if this is fully optimized to take advantage or built to understand mobile usage… something seems off"* and *"Also crashed on mobile using 3d view on iPhone."* Two causes, one gap. (i) Field mode parks the docks below the screen with `translateY(105%)`, which still counts as scrollable overflow — 453 px of it — and iOS scrolls the page itself to lift the gate's password field above the keyboard, taking every pinned bar with it. The fix is `body{position:relative}` (which finally makes body the docks' containing block, so its own overflow applies) with `overflow:clip` beside it — measured three ways, because root `overflow:clip` ALONE still lets Chromium scroll the viewport — plus a runtime scroll guard for iOS 15. (ii) The folder build parsed the ~50 MB `FIELD_EXCLUDE` drops and then drew a 178-megapixel drape, and iOS kills a tab near a gigabyte. The folder build now loads exactly the field build's payloads on a phone and caps drape textures at 2,048 px: 484 → 326 MB of heap after boot, 532 → 367 MB after opening 3D, and the drape 178.4 → 3.8 MP. **Nothing tested the folder build at phone size**, which is the one corner of the matrix the team actually uses; `test/e2e_phone.mjs` / step `phone:http` is that corner now |
| **The four areas are the CATCHMENT, not the first capture** (v22 §C) | He asked for "the area whose flow goes into the Herman impoundment … so we can understand the catchment for the Herman impoundment". The drainage card's own 37.9 ac is the ground that reaches the impoundment WITHOUT pausing in a smaller depression first — a property of the label scheme, not a catchment; the site has 38,994 lidar depressions and about 1,900 of them are real, so most of the water arrives having filled two or three. The catchment is 84.1 ac, and the v19 accumulation at the same pipe says 84.09 independently. Both numbers are printed and the card says which is which |
| **And they are computed by un-merging the outfalls, not by a new kernel** (v22 §C) | v22 §S made the three pipes in the Clear Lake trench ONE outlet on purpose, so the map keeps reporting the 282.0 ac he already reads. "Where the water goes" hands the SAME kernel the SAME list with the merge switched off (`mapConduits(cds, { mergeOutfalls: false })`), so each pipe terminates its own chain. Only the outlet naming differs — the two runs' outlets add up to the square foot, which is the assertion, not the claim |
| **The drape resolution is a texture budget, not the mesh level** (v22 §G) | His words: *"the entire site topo looks a bit pixelated when zooming in"*. Since v20 each terrain tile carried the aerial at its OWN quadtree level, so a 4-ft tile was draped at four feet per pixel. Each tile now takes its imagery two levels finer (a tablet one, a phone none — the phone keeps exactly what it had) and stitches the pieces into one texture: 1 ft/px over the mine area against 1/2/4/8 before, 22-50 MB for the visible set against a 150 MB budget. k = 2 was kept because the frame cost was MEASURED across k = 0/1/2 and does not move (1,472 / 1,383 / 1,431 ms under software GL) — the price is 21 MB of texture, and `SBMM.view.pref("drapeK")` is how to sweep it again |
| **The tile mesh is built in the worker, and there is only one copy of the loop** (v22 §G) | *"graphics are rendering a bit slow when I move around"*. The geometry build moved into the pooled tile-decode worker as `demTileMeshMain`, stringified into the same Blob worker the terrain-RGB decode already uses — the same technique, and the same reason: over `file://` nothing can be fetched. The main-thread cost of a rebuild fell from 45-130 ms to 20-80 ms and a geometry cache makes a return to a view cost nothing (16 of 16 tiles hit). **The honest half**: on the build box the longest main-thread task after a camera move is still the RENDER, at 1.2-4.0 s a frame under SwiftShader; the terrain was already yielding into ~10 ms pieces. `SBMM_GPU=1 node test/run.mjs --only terrain3d:folder,perf` on his own machine is where those numbers mean what they say |
| **A software rasteriser is named as one** (v22 §G) | *"seems like the desktop isn't taking full advantage of the GPU"* — which the app had no way to answer. `WEBGL_debug_renderer_info` names the renderer and the Help line says "(software)" for SwiftShader / llvmpipe / Mesa OffScreen / Microsoft's basic renderer. It decides exactly one thing (mesh detail starts at `high` on a desktop with a real card, a remembered choice still winning) and must not reach the drape budget or the pixel ratio, or the harnesses here would be measuring a different app from the one that ships |
| **The 2D map at high zoom was checked and is correct** (v22 §G) | The stack over the mine window at zoom 3, by its own feet-per-pixel: 3-in ortho on top, then 6-in, then the 1-ft hillshade, then the 2-ft one, and nothing is zoom-gated. What is seen past zoom 4 is the 3-inch photograph magnified past its own resolution. The one real 2D finding is `docs/ALIGNMENT_REPORT.md`'s: the hillshade JPEGs are drawn over an AREA rectangle while the DEM is a NODE grid — a display-only half-cell offset, two lines in `js/layers.js`, deliberately left for its own commit |
...
| **The black band around the mine area was a PAYLOAD defect, and it is fixed in the payload** (v24, 2026-09-18) | *"there is a black boundary between how you are stitching the mine area to the rest, a black border which sometimes goes away."* `tools/build_tiles.py` cut the fine ortho levels from the fine sources ALONE and started each tile at zeros, so every pixel of an edge tile outside the 6-in photography was (0,0,0) — 177 black columns in one tile, 127 black rows in another — and the renderer composites the fine tile over the stretched coarse ancestor, so the black won. `ortho_mine`'s rectangle IS `dem_abp`'s, which is why it traced the 1-ft window on all four sides; "sometimes goes away" is the LOD, since only a DEM tile fine enough to be given a fine ortho drapes from one. Every source is painted now, coarsest first, and the tile is written only where a fine one reaches. 158 ft of continuous black on the east edge before, 6 ft after. The single-file builds never had it — they synthesise their tiles from the resident imagery |
| **The 3D drape reaches the photography's own resolution — negative pyramid levels** (v24, 2026-09-18) | *"the entire site topo looks a bit pixelated when zooming in."* The 3D drape floored at 1 ft/px while the 2D map has always drawn the same ground at 6 in and 3 in. `cell = 2**z`, so the fine levels are z **-1** (0.5 ft/px over the mine ortho) and z **-2** (0.25 over the ABP crop): +12.2 MB of payload, 52.0 → 64.2. `drapeK` still bounds the TEXTURE at 256·2^k px, so no texture grew and the drawn-set cost is 69-88 MB against a 150 MB budget. **The phone and tablet caps are untouched** — the sharper drape is desktop only, which is what keeps the phone, field and tablet harnesses measuring the same app |
| **A selection change does not rebuild the 3D scene** (v24, 2026-09-18) | *"there seems to be a rendering issue with how smoothly I can view things in 3D."* Measured with the new `SBMM.viewer3d.frameStats()`: a click rebuilt every overlay in the view — 4,085-5,971 draped polylines and 425,000-913,000 ground elevations, 76-141 ms of main thread — to change the colour and width of one line. The overlay is two groups now, the read-only project data and the user's own features, and a selection or an edit rebuilds only the second: 0.6-1.7 ms. A module whose own data changed still asks for a full rebuild through `SBMM.viewer3d.refreshOverlays()` |
| **The desktop pixel ratio is capped at 1.5** (v24, 2026-09-18) | A 4K screen reports `devicePixelRatio` 2, so the renderer drew four times the pixels of its own canvas — and the drape, the overlay lines and the sky are all fill-bound. `SBMM.view.pref("pixelRatio")` remembers a different value (1..3) for a machine that wants it. The phone and tablet rule stays `min(2, dpr)` exactly as it was |
| **The 3D diagnostics open with one line of numbers** (v24, 2026-09-18) | GPU frame time cannot be judged on the build box (SwiftShader), and an adjective is not a measurement. **View settings → copy 3D diagnostics** now leads with frames, render ms, label and particle cost, hover raycasts, overlay rebuilds (and how many were full), terrain selects, the pixel ratio, the drape's ft/px and the card's name — so the next report from his machine arrives with numbers rather than a word |
...
0b. **Reconcile the flagged boring contacts — he logged 18 of the 44 holes himself.**
   The 2025 OpenGround logs shipped in v9.25 and every hole carries two statements of the
   waste/native contact: the logger's remark on the rig and the base of the deepest unit
   described as WASTE. **18 of the 44 disagree** — 17 between the remark and the strata,
   3 where waste is logged BELOW native (SB-7, SB-43, SB-45; two of those also differ).
   The older field spreadsheet was a third statement and is retired (decision above).
   The app resolves none of them on purpose:
   it draws both lines on the strip log, flags each difference, and `LOGS` is the
   44-row table with the two numbers side by side, sortable, with copy CSV. **This is a
   reading task, not a code task** — M. Sharif logged 18 of the 44,
...
0c. **Site lithology / geologic maps as a layer — LOW priority (2026-09-09).** The
   engineer's own words: "use that and knowledge of the site lithology maps you can find
   online and maybe bring those in, but that bringing over lithology maps is not high
   priority." The candidates are the USGS geologic mapping of the Clear Lake volcanic field
   (Hearn, Donnelly-Nolan & Goff — the Sulphur Bank andesite, the lake sediments, the
   Franciscan basement) and the CGS/USGS quadrangle sheets; any of them would arrive as a
   registered raster or a small polygon payload under Site framework, the same way the EA
   sheets did, and would be a display layer only — it must not become an input to a
   contact number. Nothing is built; do it only after the items above it.
...
14. **CLOSED (ruling, 4 Sep 2026) — the sunken pipe mouth.** The raindrop and the
   ...
   `herman_pipe_s` → `herman_main_s` → the outfall
   (812.2 ft in pipe), against the overtopping card's first discharge at 1,341.55 — the two
